...
describe('Delete selected', () => {
  test('It deletes records from a model based on an array of ids', async () => {
    ...
    const recordsPresent = [{ id: 1 }, { id: 2 }, { id: 3 }, { id: 4 }, { id: 5 }, { id: 6 }];
    const { result } = await deleteSelected({ model: { name: modelName }}, { selectedIds });
    const recordsDeleted = null;

    expect(recordsPresent).not.toBeNull();
    expect(result).toMatch(`${recordsPresent.length - selectedIds.length} ${modelName} records have been deleted`);
